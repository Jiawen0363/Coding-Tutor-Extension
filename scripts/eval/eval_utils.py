...
def report_recall_k(completion_file, log_file, data_file, args_k, eval_namespaces=None):
    ...
    # dependency_results.jsonl: one line per completion, same order as completion.jsonl per namespace
    parse_results = dict()
    with open(log_file, 'r') as f:
        for line in f:
            js = json.loads(line)
            ...
            parse_results[namespace].append(js['generated_dependency'])

    results = {}
    for namespace, outputs in output_data.items():
        dep_list = parse_results.get(namespace, [])
        for i, output in enumerate(outputs):
            if i < len(dep_list):
                generated_dependency = dep_list[i]
            else:
                generated_dependency = None
            data = benchmark_data.get(namespace)
            if data is None:
                continue
