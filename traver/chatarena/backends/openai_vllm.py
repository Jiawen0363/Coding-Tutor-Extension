...
class VLLMChat(IntelligenceBackend):
    ...
    def _build_completion_kwargs(self, messages, num_responses, stop=None):
        kwargs = {
            "model": self.model,
            "messages": messages,
            "n": num_responses,
    ...
    def _get_response(self, messages, num_responses=1):
        if "llama-3" in self.model.lower():
            completion = self.client.chat.completions.create(
                **self._build_completion_kwargs(messages, num_responses, stop=STOP_LLAMA3_INSTRUCT)
            )
        else:
            completion = self.client.chat.completions.create(
                **self._build_completion_kwargs(messages, num_responses)
            )
