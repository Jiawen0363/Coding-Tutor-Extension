...
def get_parser():
    ...
    parser.add_argument('--model_type', type=str, default='gpt', choices=['gpt', 'lm']) 
...
# 检查输入文件是否存在，如果不存在则尝试创建目录结构
if not os.path.exists(args.completion_file):
    # 尝试创建目录结构
    input_dir = os.path.dirname(args.completion_file)
    if input_dir:
    ...
    # 再次检查文件是否存在
    if not os.path.exists(args.completion_file):
        print(f"Warning: Input file {args.completion_file} still does not exist after creating directory.")
        print("This might indicate that the file generation step needs to be run first.")
        # 可以选择退出或者继续处理其他文件
        exit(1)
...
# 使用 tree_sitter_python 而不是手动加载语言库
...
if __name__ == "__main__":
    ...
    with open(args.completion_file, "r") as f_r, open(args.output_file, "w") as f_w:
        for line in f_r:
            ...
            namespace, completions = extract_body_gpt(data)
            ...
            for completion in completions:
                ...
                f_w.write(json.dumps(json_line) + "\n")
                idx += 1
    print(f"Total number of completions: {idx}")
