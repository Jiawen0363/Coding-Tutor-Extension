...
class Verifier(PeftModel):
    ...
    def __init__(self, model: PreTrainedModel, lora_config: LoraConfig, torch_dtype=torch.float):
        ...
        # Ensure all new modules are on the same device as base_model
        # Get device from base_model's first parameter
        if hasattr(self.base_model, 'get_input_embeddings'):
            try:
                base_device = next(self.base_model.get_input_embeddings().parameters()).device
                # Use data attribute to move Parameter data to device
                self.gain.data = self.gain.data.to(base_device)
                self.bias.data = self.bias.data.to(base_device)
                self.vscore_head = self.vscore_head.to(base_device)
            except (StopIteration, AttributeError):
                # If we can't determine device, let it be handled in forward
                pass
    ...
    def transform(self, last_hidden_states):
        ...
        # Ensure gain and bias are on the same device as input
        gain = self.gain.to(last_hidden_states.device)
        bias = self.bias.to(last_hidden_states.device)
        return gain * last_hidden_states + bias
    ...
    def forward(self, input_ids, attention_mask, labels=None, **kwargs):
        ...
        # Ensure vscore_head is on the same device as input
        device = v_hidden_states.device
        vscore_head = self.vscore_head.to(device)
        v_scores = vscore_head(self.dropout(v_hidden_states)).squeeze(-1)
