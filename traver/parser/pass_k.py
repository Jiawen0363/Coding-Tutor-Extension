...
def read_captured_output(output_file):
    output_file.seek(0)
    output = output_file.read().decode('utf-8', errors='replace')
    if len(output) > MAX_ERROR_LOG_CHARS:
        return f'[truncated to last {MAX_ERROR_LOG_CHARS} chars]\n' + output[-MAX_ERROR_LOG_CHARS:]
    return output


def build_error_log(reason, test, project_path, stdout_file=None, stderr_file=None, return_code=None, exception=None):
    error_log = {
        'reason': reason,
        'test': test,
    ...
    if stdout_file is not None:
        error_log['stdout'] = read_captured_output(stdout_file)
    if stderr_file is not None:
        error_log['stderr'] = read_captured_output(stderr_file)
    if exception is not None:
        error_log['exception'] = repr(exception)
    return error_log
...
def execution_tests(test, project_path):
    ...
    with tempfile.TemporaryFile() as stdout_file, tempfile.TemporaryFile() as stderr_file:
        process = subprocess.Popen(['bash', '-c', command], cwd=project_path, stdout=stdout_file, stderr=stderr_file)
        try:
            while True:
                ...
                if process_memory > 5 * 1024 * 1024 * 1024: # 5GB memory usage per test
                    ...
                    return False, build_error_log('out_of_memory', test, project_path, stdout_file, stderr_file)
                return_code = process.poll()
                if return_code is not None:
                    if return_code != 0:
                        process.wait()
                        print(f"Execution Error with return code {return_code}: {project_path} ")
                        return False, build_error_log('execution_error', test, project_path, stdout_file, stderr_file, return_code)
                    else:
                        break
            ...
            return False, build_error_log('other_error', test, project_path, stdout_file, stderr_file, exception=e)
        finally:
            if process.poll() is None:
                process.terminate()
                process.wait()
...
def check_correctness(args, data):
    ...
    if completion == "    pass\n":
        return 'Fail', build_error_log('empty_pass_completion', None, None)
    ...
    try:
        for test in data['tests']:
            try:
                result, error_log = execution_tests(test, project_path)
                if not result:
                    flag = 'Fail'
                    break
            except func_timeout.exceptions.FunctionTimedOut:
                flag = 'Fail'
                error_log = build_error_log('timeout', test, project_path)
                error_log['timeout_seconds'] = 20
    ...
    with open(args.log_file, 'a') as f:
        for output in tqdm(todo_output_data):
            ...
            if namespace in benchmark_data:
                ...
                result, error_log = check_correctness(args, data)
                output['Result'] = result
                if error_log is not None:
                    output['error_log'] = error_log
