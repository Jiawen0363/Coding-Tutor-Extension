...
def parse_args():
    ...
    parser.add_argument(
        "--tutor_prompt_setting",
        type=str,
        default="base",
        choices=["base", "student", "privileged", "teacher"],
        help="Tutor system prompt: base (legacy), student (deployed OPD), privileged (z only), teacher (student+z).",
    )
    ...
    parser.add_argument('--tutor_vllm_api_key', type=str, default=None)
    parser.add_argument('--student_vllm_api_key', type=str, default=None)
    ...
    parser.add_argument("--max_samples", type=int, default=None,
                        help="If set, run at most this many dialogues for the current student level.")
...
def interactive_simulation(args, prompt_data, tutor_backend, student_backend, moderator_backend):
    ...
    with open(output_path, "a", encoding='utf-8') as fw:
        for js in tqdm(prompt_data):
            ...
            if args.tutor_setting in ("vanilla", "privileged_teacher"):
                tutor = Player(
...
def main(args):
    ...
    tutor_vllm_api_key = args.tutor_vllm_api_key or args.vllm_api_key
    student_vllm_api_key = args.student_vllm_api_key or args.vllm_api_key
    prompt_elements = load_json_data(args.prompt_element_file)
    tokenizer = tiktoken.encoding_for_model("gpt-4")

    prompt_data = []
    for d in tqdm(prompt_elements):
        _priv_levels = ("privileged", "teacher")
        tutor_desc = prompt_tutor(
            d,
            tokenizer,
            setting=args.tutor_prompt_setting,
            max_code_context=args.max_code_context,
            student_level=args.student_setting if args.tutor_prompt_setting in _priv_levels else None,
        )
    ...
    if args.tutor_model_name_or_path in ("gpt-3.5", "gpt-4", "gpt-4o"):
        ...
        tutor_backend = VLLMChat(
            vllm_api_key=tutor_vllm_api_key,
    ...
    student_backend = VLLMChat(
        vllm_api_key=student_vllm_api_key,
...
if __name__ == '__main__':
    ...
    for key in ("vllm_api_key", "tutor_vllm_api_key", "student_vllm_api_key"):
        if safe_args.get(key):
            safe_args[key] = "***"
    print(argparse.Namespace(**safe_args))
    main(args)
